	...
	if (globalThis['html5'] !== undefined) {
		...
		if (down !== undefined) {
			...
			keys_down.forEach((key) => {
				...
				if (this.loaded && pressed) {
					const route = Object.keys(ROUTES).find(
						(route) => ROUTE_KEYS[route] === this.loaded,
					);
					if (route !== undefined)
						msg.post(ROUTES[route], 'on_virtual_input', {
							action_id: hash(key.trim()),
							action: { pressed: true, released: false },
						});
				}
			});
			...
			['up', 'right', 'down', 'left', 'accept', 'back'].forEach((key) => {
				const released =
					this.prev_down.includes(key) && !keys_down.includes(key);
				if (this.loaded && released) {
					...
					if (route !== undefined)
						msg.post(ROUTES[route], 'on_virtual_input', {
							action_id: hash(key.trim()),
							action: { pressed: false, released: true },
						});
				}
			});
