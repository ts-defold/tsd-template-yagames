...
export function on_input(this: props, action_id: hash, action: Action): void {
	if (
		(action_id === hash('start') || action_id === hash('back')) &&
		action.pressed
	) {
...
function requestLeaderboard(props: props, mode: 'top' | 'around'): void {
	...
	yagames.leaderboards_get_entries(
		'highscore',
		{ includeUser: mode === 'around', quantityTop: 5 },
		(_ctx, err, data) => {
			props.pending = false;
			if (err !== undefined) {
				updateLeaderboard(props, true);
...
function updateLeaderboard(props: props, offline = false): void {
	...
	for (let i = 0; i < 5; i++) {
		...
		gui.set_text(
			initials_node,
			i < db.scores.length ? db.scores[i].initials : '',
		);
		gui.set_text(
			score_node,
			i < db.scores.length ? db.scores[i].score.toString() : '',
		);
	}
}
