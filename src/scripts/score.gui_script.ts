...
export function init(this: props): void {
	...
	this.nodes = {
		...
		'cursor-1': {
			'up-1': gui.get_node('up-1'),
			'down-1': gui.get_node('down-1'),
		},
		'cursor-2': {
			'up-2': gui.get_node('up-2'),
			'down-2': gui.get_node('down-2'),
		},
		'cursor-3': {
			'up-3': gui.get_node('up-3'),
			'down-3': gui.get_node('down-3'),
		},
	};

	// Set initial state
	gui.set_text(this.nodes.score, '');
	Object.keys(this.nodes['cursor-1']).forEach((k) =>
		gui.set_enabled(this.nodes['cursor-1'][k], true),
	);
	Object.keys(this.nodes['cursor-2']).forEach((k) =>
		gui.set_enabled(this.nodes['cursor-2'][k], false),
	);
	Object.keys(this.nodes['cursor-3']).forEach((k) =>
		gui.set_enabled(this.nodes['cursor-3'][k], false),
	);
}
...
export function on_input(this: props, action_id: hash, action: Action): void {
	if (action_id === hash('up') && action.pressed) {
		...
		this.letters_index =
			(this.letters_index - 1 + this.letters.length) % this.letters.length;
		fx.menu_item();
	} else if (action_id === hash('left') && action.pressed) {
		this.initials_index =
			(this.initials_index - 1 + this.initials.length) % this.initials.length;
		this.letters_index = this.letters.indexOf(
			this.initials[this.initials_index],
		);
		fx.press();
		...
		action_id === hash('accept') ||
		(action_id === hash('start') && action.pressed)
	) {
	...
	Object.keys(this.nodes['cursor-1']).forEach((k) =>
		gui.set_enabled(
			this.nodes['cursor-1'][k],
			this.initials_index === 0 ? true : false,
		),
	);
	Object.keys(this.nodes['cursor-2']).forEach((k) =>
		gui.set_enabled(
			this.nodes['cursor-2'][k],
			this.initials_index === 1 ? true : false,
		),
	);
	Object.keys(this.nodes['cursor-3']).forEach((k) =>
		gui.set_enabled(
			this.nodes['cursor-3'][k],
			this.initials_index === 2 ? true : false,
		),
	);
}
...
export function on_message(
	...
	if (message_id === hash('on_virtual_input')) {
		const { action_id, action } = message as {
			action_id: hash;
			action: Action;
		};
